...
func Mtf_GetInvoice(c *gin.Context) {
...
func mtf_create_invoice(w io.Writer, invoice_id *int64, inv *invoice_item, finance *models.Finance, orders []order_unit) (err error) {
	...
	for i := 0; i < len(orders); i++ {
		...
		p.MultiCell(box6-1, 5, fmt.Sprintf("%s\n-%s\n%s",
			utils.FormatNumber(subtotal),
			utils.FormatNumber(pajak),
			utils.FormatNumber(subtotal-pajak),
		), "0", "RM", false)
		...
		p.CellFormat(box5-1, lh, utils.FormatNumber(subtotal), "0", 1, "RM", false, 0, "")
	...
	p.SetFont(font, "B", 10)
	p.CellFormat(box6, lh, fmt.Sprintf("%s ", utils.FormatNumber(total_finance)), "1", 1, "RM", false, 0, "")

	p.SetFont(font, "", 10)
	p.CellFormat(box1+box2+box3+box4+box5, lh, "  Pajak", "1", 0, "LM", false, 0, "")
	//p.CellFormat(box5, lh, " ", "1", 0, "LM", false, 0, "")
	p.SetFont(font, "B", 10)
	p.CellFormat(box6, lh, fmt.Sprintf("-%s ", utils.FormatNumber(total_pajak)), "1", 1, "RM", false, 0, "")

	p.SetFont(font, "", 10)
	p.CellFormat(box1+box2+box3+box4+box5, lh, "  Total", "1", 0, "LM", false, 0, "")
	//p.CellFormat(box5, lh, " ", "1", 0, "LM", false, 0, "")
	p.SetFont(font, "B", 10)
	p.CellFormat(box6, lh, fmt.Sprintf("%s ", utils.FormatNumber(total)), "1", 1, "RM", false, 0, "")
...
func mft_create_lampiran1(
	...
	html.Write(lineHt, fmt.Sprintf("Dengan total biaya <b>Rp%s,-</b> (<i>** %s Rupiah</i>)",
		utils.FormatNumber(*total), strings.TrimSpace(fyc.Terbilang(*total))))
...
serta %s terbebas dari segala tuntutan maupun permasalahan yang terjadi di kemudian hari.`,
			utils.FormatNumber(*total), finance.Name, finance.Name))
...
func mft_create_lampiran2(
	...
	/*
	   	...
	   	lineHt += 2
	   	html.Write(lineHt, fmt.Sprintf("Dengan total biaya <b>Rp%s,-</b> (<i>** %s Rupiah</i>)",
	   		utils.FormatNumber(*total), strings.TrimSpace(convertion.Terbilang(*total))))
