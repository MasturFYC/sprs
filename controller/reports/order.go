...
func ReportOrderAllWaiting(c *gin.Context) {
...
func rptOrder(filter *report_order_request) ([]order_invoiced, error) {
	...
	var qWheel = controller.NestQuerySingle(`SELECT name, short_name as "shortName" FROM wheels WHERE id = t.wheel_id`)
	var qMerk = controller.NestQuerySingle("SELECT name FROM merks WHERE id = t.merk_id")

	var qTye = controller.NestQuerySingle(fmt.Sprintf(`SELECT t.name, %s AS wheel, %s AS merk FROM types t WHERE t.id = u.type_id`,
		qWheel, qMerk,
	))

	var qUnit = controller.NestQuerySingle(fmt.Sprintf(`SELECT u.nopol, u.year,
		%s AS type
		FROM units u
		WHERE u.order_id = t.id`,
		qTye))

	var qFinance = controller.NestQuerySingle(`SELECT f.name, f.short_name "shortName" FROM finances f WHERE f.id = t.finance_id`)
	var qBranch = controller.NestQuerySingle(`SELECT b.name FROM branchs AS b WHERE b.id = t.branch_id`)
