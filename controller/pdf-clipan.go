...
func Clipan_GetInvoice(c *gin.Context) {
...
func clipan_create_invoice(w io.Writer, invoice_id *int64, inv *invoice_item, finance *models.Finance, orders []order_unit_customer) (err error) {
	...
	for i := 0; i < len(orders); i++ {
		...
		p.CellFormat(box3, lh, fmt.Sprintf("Rp %s   ", utils.FormatNumber(subtotal)), "0", 1, "RM", false, 0, "")
	}
	...
	p.SetFont(font2, "B", 12)
	p.CellFormat(box3, lh, fmt.Sprintf("Rp %s   ", utils.FormatNumber(total)), "1", 1, "RM", false, 0, "")
